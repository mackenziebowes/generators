...
export const Badge = (props: BadgeProps) => {
	...
	const badgeCn = cn([
		"inline-flex items-center justify-center rounded-md border px-2 py-0.5 text-xs border-[2px] w-fit h-fit",
		{
			...
			"bg-(--color1-700)/90 text-(--color1-100) border-(--color1-700) font-bold":
				priority == "med" && (type == "destructive" || type == "error"),
			"bg-(--color2-700)/90 text-(--color2-100) border-(--color2-700) font-bold":
				priority == "med" && type == "success",
			"bg-(--color3-700)/90 text-(--color3-100) border-(--color3-700) font-bold":
				priority == "med" && type == "warning",
			"bg-(--color4-700)/90 text-(--color4-100) border-(--color4-700) font-bold":
				priority == "med" && type == "info",
			"bg-(--surface-500)/25 text-(--marking-600) border-(--marking-600) font-bold":
				priority == "low" && type == "default",
			"bg-(--surface-500)/25 text-(--color1-600) border-(--color1-600) font-bold":
				priority == "low" && (type == "destructive" || type == "error"),
			"bg-(--surface-500)/25 text-(--color2-600) border-(--color2-600) font-bold":
				priority == "low" && type == "success",
			"bg-(--surface-500)/25 text-(--color3-600) border-(--color3-600) font-bold":
				priority == "low" && type == "warning",
			"bg-(--surface-500)/25 text-(--color4-600) border-(--color4-600) font-bold":
